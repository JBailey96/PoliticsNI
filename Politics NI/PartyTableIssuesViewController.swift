...
class PartyTableIssuesViewController: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCellWithIdentifier("cell10", forIndexPath: indexPath) as! PartyCellIssuesViewController
        let entry = partyViewsArray[indexPath.row]
        
        cell.issueLabel.text = entry.issueDesc
        return cell
    }
    
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        let viewIndex = tableView.indexPathForSelectedRow?.row
        if segue.identifier == "seeMoreDetail" {
            let desView = segue.destinationViewController as! PartyIssuesViewController
            desView.partyViewsArray.append(partyViewsArray[viewIndex!])
        }
    }
